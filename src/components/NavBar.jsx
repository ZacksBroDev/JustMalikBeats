    ...
    <header className="blog-header">
      ...
      <nav className={`navbar ${isMobileMenuOpen ? 'mobile-open' : ''}`}>
        <ul>
          ...
          {isLoggedIn ? (
            <>
              <li><Link to="/account" onClick={closeMobileMenu}>My Account</Link></li>
              <li className="user-menu">
                <span className="user-name">Hi, {currentUser?.name?.split(' ')[0]}!</span>
                <button onClick={() => { logout(); closeMobileMenu(); }} className="logout-link">Sign Out</button>
              </li>
            </>
          ) : (
            <li><button onClick={() => { openLoginModal(); closeMobileMenu(); }} className="login-link">Sign In</button></li>
          )}
